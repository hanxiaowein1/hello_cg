...
void grid3d::update_matrices() {
	if (this->Ang[0] != 90 || this->Ang[1] != 90 || this->Ang[2] != 90) {
		nonortho = 1;
	...
	for (unsigned int k = 0; k <= N[2]; ++k)
		for (unsigned int j = 0; j <= N[1]; ++j) {
			if (G.x_data > 1)
				...
				// memcpy((*F)[k][j].data(), (*G.F)[k][j].data(), (N[0] + 1)*sizeof(GRD_data_type));
				(*F)[k][j] = (*G.F)[k][j];
		}
}
...
int grid3d::alloc_F() {
	...
	try
	{
		// F = std::vector<std::vector<std::vector<GRD_data_type>>>(N[2] + 1, std::vector<std::vector<GRD_data_type>>(N[1] + 1, std::vector<GRD_data_type>(N[0] + 1)));
		F = make_shared<std::vector<std::vector<std::vector<GRD_data_type>>>>(
			N[2] + 1, std::vector<std::vector<GRD_data_type>>(N[1] + 1, std::vector<GRD_data_type>(N[0] + 1))
		);
	}
	catch(...)
	{
		std::cout << "F allocate memory failed" << std::endl;
		return -1;
	}
	// if (!F)
	// 	return -1;
...
int grid3d::set_data_pointer(unsigned int Nx, unsigned int Ny, unsigned int Nz, GRD_data_type* data) {
	...
	try
	{
		// F = std::vector<std::vector<std::vector<GRD_data_type>>>(Nz);
		F = std::make_shared<std::vector<std::vector<std::vector<GRD_data_type>>>>(Nz);
	}
	...
	for (unsigned int k = 0; k < Nz; ++k) {
		...
		try
		{
			(*F)[k] = std::vector<std::vector<GRD_data_type>>(Ny);
		}
		catch(const std::exception& e)
		{
			F = {};
		...
		{
			// (*F)[k][j] = data + j*Nx;
			(*F)[k][j] = std::vector<GRD_data_type>(data, data + Nx);
		}
		data += Ny*Nx;
	}
	for (int i = 0; i != 3; ++i) {
...
int grid3d::add_subgrid(unsigned int Oi, unsigned int Oj, unsigned int Ok,
	...
	try
	{
		// G->F = std::vector<std::vector<std::vector<GRD_data_type>>>(Nk);
		G->F = std::make_shared<std::vector<std::vector<std::vector<GRD_data_type>>>>(Nk);
	}
	catch(const std::exception& e)
	{
		G->F = {};
	...
	for (unsigned int k = 0; k < Nk; ++k) {
		...
		try
		{
			// G->(*F)[k] = std::vector<std::vector<GRD_data_type>>(Nj);
			(*G->F)[k] = std::vector<std::vector<GRD_data_type>>(Nj);
		}
		catch(const std::exception& e)
		{
		...
		for (unsigned int j = 0; j < Nj; ++j)
		{
			...
			(*G->F)[k][j] = std::vector<GRD_data_type>((*this->F)[Ok + k*Sk][Oj + j*Sj].data() + Oi, (*this->F)[Ok + k*Sk][Oj + j*Sj].data() + Oi + Ni);
		}
	}
	G->d[0] = Si*d[0];
	G->d[1] = Sj*d[1];
...
#else
			in.read(reinterpret_cast<char*>((*F)[k][j].data()),(N[0] + 1)*sizeof(float));
#endif
	return (in.good()? 0: -4);
}
//******************************************************************
...
int grid3d::read_scanfiles(const char *filename, unsigned int res, int order) {
	...
	while (1) {
		...
		if (!((++k)&63)) {
			...
			try
			{
				// auto pt = std::vector<std::vector<std::vector<GRD_data_type>>>(k + 64);
				auto pt = std::make_shared<std::vector<std::vector<std::vector<GRD_data_type>>>>(k + 64);
				if(F)
				{
					// memcpy(pt->data(), F->data(), k * sizeof(void*));
					for(unsigned int temp = 0; temp < k; temp++)
					{
						pt->emplace_back(std::move((*F)[temp]));
					}
					// F = {};
					F.reset();
				}
			...
			// }
			// if (F.size()) {
			// 	memcpy(pt, F.data(), k*sizeof(void*));
			// 	delete[] F;
			// }
			// F = pt;
		}
		...
		try
		{
			(*this->F)[k] = std::vector<std::vector<GRD_data_type>>(res);
		}
		catch(const std::exception& e)
		{
			// std::cerr << e.what() << '\n';
		...
		for (j = 0; j != res; ++j)
			...
			(*this->F)[k][j] = std::vector<GRD_data_type>(res);
		if (!(*this->F)[k][N[1]].size()) {
			m = -2;
			break;
		}
...
int grid3d::read_raw_file(const char *filename, unsigned int *n, int byte, int isfloat) {
...
#endif
	{
		...
		for (k = 0; k != n[2]; ++k)
			for (j = 0; j != n[1]; ++j)
				in.read(reinterpret_cast<char*>((*F)[k][j].data()), byte);
	} else if (isfloat) {
		if (byte == 8) {
	...
	for(unsigned int k = 0; k < nz; k++)
	{
		for(unsigned int j = 0; j < ny; j++)
		{
			for(unsigned int i = 0; i < nx; i++)
			{
				...
				(*F)[k][j][i] = std::sqrt(square_x + square_y + square_z) - radius;
			}
		}
	}
...
int grid3d::read_dat_file(const char *filename) {
	...
	unsigned int abbrevation_x = nx / step;
	unsigned int abbrevation_y = ny / step;
	unsigned int abbrevation_z = nz / step;
	this->N[0] = abbrevation_x - 1;
	this->N[1] = abbrevation_y - 1;
	this->N[2] = abbrevation_z - 1;
	...
	for(unsigned int k = 0; k < abbrevation_z; k++)
	{
		for(unsigned int j = 0; j < abbrevation_y; j++)
		{
			for(unsigned int i = 0; i < abbrevation_x; i++)
			{
				in.seekg((k - 1 < 0 ? 0 : k * step) * nx * ny + (j - 1 < 0 ? 0 : j * step) * nx + i * step);
				in.read(reinterpret_cast<char*>(&n), sizeof(short int));
				(*F)[k][j][i] = n;
			}
		}
...
int grid3d::save_raw_file(const char *filename) {
	...
	for (unsigned int k = 0; k <= N[2]; ++k)
		for (unsigned int j = 0; j <= N[1]; ++j) {
			if (x_data > 1)
				for (unsigned int i = 0; i <= N[0]; ++i)
					out.write(reinterpret_cast<char*>((*F)[k][j].data() + i*x_data), sizeof(GRD_data_type));
			else
				out.write(reinterpret_cast<char*>((*F)[k][j].data()), (N[0] + 1)*sizeof(GRD_data_type));
		}
	return (out.good()? 0: -4);
}
