...
private:
	// GRD_data_type ***F;
	// std::vector<std::vector<std::vector<GRD_data_type>>> F;
	std::shared_ptr<std::vector<std::vector<std::vector<GRD_data_type>>>> F;
...
public:
	...
	void* get_vertices_pointer();
	void* get_triangle_index_pointer();
	...
	void save_as_obj(std::string filename = "./default.obj");
friend MC33;
};
...
private:
	...
	// const std::vector<std::vector<std::vector<GRD_data_type>>> *F = nullptr;
	std::shared_ptr<std::vector<std::vector<std::vector<GRD_data_type>>>> F;
	...
	std::vector<std::vector<unsigned int>> Dx, Dy, Ux, Uy, Lz;
	MC33_real *v;
