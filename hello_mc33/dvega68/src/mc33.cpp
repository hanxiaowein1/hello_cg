...
void MC33::find_case(unsigned int x, unsigned int y, unsigned int z, unsigned int i) {
	...
	while (i)
	{
		...
		for (k = 3; k;) {
			...
			{
				switch (c)
				{ // the vertices r[3] and normals (r + 3)[3] are calculated here
				// NOTE: please rename normals to normalize...
				case 0:
					if (z || x)
					{
						ti[--k] = p[0] = this->Dy[y][x];
					}
					else
					{
						if (this->v[0] == 0)
						...
						{
							t = this->v[0] / (this->v[0] - this->v[1]);
							r[0] = r[2] = 0;
							r[1] = y + t;
							// NOTE: this is maybe sign distance function normal?
							r[3] = (v[4] - v[0])*(1 - t) + (v[5] - v[1])*t;
							r[4] = v[1] - v[0];
							r[5] = (v[3] - v[0])*(1 - t) + (v[2] - v[1])*t;
							p[0] = store_point(r);
						}
						this->Dy[y][0] = ti[--k] = p[0];
					}
					break;
				case 1:
					if (x)
		...
		// NOTE: you must be kidding me, how index could be the same?
		if (ti[0] != ti[1] && ti[0] != ti[2] && ti[1] != ti[2]) { //to avoid zero area triangles
			if (!(S->nT&0x0FFF)) {
				try {
					// NOTE: why use resize? Let vector do it self.
			...
			if(ti[0] > 1000000 || ti[1] > 1000000 || ti[2] > 1000000)
			{
				std::cout << "take attention! triangle index are abnormal" << std::endl;
			}
...
int MC33::set_grid3d(grid3d &G) {
	...
	if (G.nonortho) {
		store_point = [this] (MC33_real* r) {
			...
			multAbf(this->_A,r,r,0);
			for (int i = 0; i != 3; ++i)
				S->V[nv].v[i] = *(r++) + this->MC_O[i];
			multAbf(A_,r,r,1);
			code_in_define_part02
		};
		for (int j = 0; j != 3; ++j)
			for (int i = 0; i != 3; ++i) {
				this->_A[j][i] = G._A[j][i]*G.d[i]; // true transformation matrices
				this->A_[j][i] = G.A_[j][i]/G.d[j];
			}
	} else
#endif
	if (G.d[0] != G.d[1] || G.d[1] != G.d[2]) {
		...
		store_point = [this] (MC33_real* r) {
			code_in_define_part01
			for (int i = 0; i != 3; ++i)
				S->V[nv].v[i] = *(r++)* this->MC_D[i] + this->MC_O[i];
			r[0] *= ca; // normal[0]
			r[1] *= cb; // normal[1]
			code_in_define_part02
		};
	...
	if (!F)
		return 0;
	try{
		// this->Lz = new (nothrow) unsigned int*[this->ny + 1]; //edges 1, 3, 5 (only write) and 7
		this->Lz = std::vector<std::vector<unsigned int>>(this->ny + 1, std::vector<unsigned int>());
		// this->Dy = new (nothrow) unsigned int*[this->ny]; //edges 0 and 4
		this->Dy = std::vector<std::vector<unsigned int>>(this->ny, std::vector<unsigned int>());
		// this->Uy = new (nothrow) unsigned int*[this->ny]; //edges 2 and 6 (only write)
		this->Uy = std::vector<std::vector<unsigned int>>(this->ny, std::vector<unsigned int>());
		// this->Dx = new (nothrow) unsigned int*[this->ny + 1]; //edges 8 and 9
		this->Dx = std::vector<std::vector<unsigned int>>(this->ny + 1, std::vector<unsigned int>());
		// this->Ux = new (nothrow) unsigned int*[this->ny + 1]; //edges 10 (only write) and 11
		this->Ux = std::vector<std::vector<unsigned int>>(this->ny + 1, std::vector<unsigned int>());
	}catch(...){
		free_temp_D_U();
		return -1;
	}
	...
	try
	{
		...
		{
			// this->Dx[j] = new (nothrow) unsigned int[this->nx];
			this->Dx[j] = std::vector<unsigned int>(this->nx, 10000000);
			// this->Ux[j] = new (nothrow) unsigned int[this->nx];
			this->Ux[j] = std::vector<unsigned int>(this->nx, 10000000);
			// this->Lz[j] = new (nothrow) unsigned int[this->nx + 1];
			this->Lz[j] = std::vector<unsigned int>(this->nx + 1, 10000000);
			// this->Dy[j] = new (nothrow) unsigned int[this->nx + 1];
			this->Dy[j] = std::vector<unsigned int>(this->nx + 1, 10000000);
			// this->Uy[j] = new (nothrow) unsigned int[this->nx + 1];
			this->Uy[j] = std::vector<unsigned int>(this->nx + 1, 10000000);
		}
	}
	catch(...)
	{
	...
	// if (this->Uy[this->ny - 1])
	if(this->Uy[this->ny - 1].size() == nx + 1)
	{
		// this->Dx[ny] = new (nothrow) unsigned int[this->nx];
		this->Dx[ny] = std::vector<unsigned int>(this->nx, 10000000);
		// this->Ux[ny] = new (nothrow) unsigned int[this->nx];
		this->Ux[ny] = std::vector<unsigned int>(this->nx, 10000000);
		// this->Lz[ny] = new (nothrow) unsigned int[this->nx + 1];
		this->Lz[ny] = std::vector<unsigned int>(this->nx + 1, 10000000);
		this->di = G.x_data > 1? G.x_data: 1;
		// if (Lz[ny])
		if(this->Lz[ny].size() == nx + 1)
		{
			std::cout << "initialize temporary cache succeed" << std::endl;
			return 0;
		}
	}
	// else
	// {
	// 	this->Dx[this->ny] = this->Ux[ny] = this->Lz[ny] = 0;
	// }
	clear_temp_isosurface();
	return -1;
}
...
int MC33::calculate_isosurface(surface &Sf, MC33_real iso) {
	...
	std::shared_ptr<std::vector<std::vector<std::vector<GRD_data_type>>>> FG = F;
	...
	Sf.iso = iso;
	memoryfault = 0;
	unsigned int d = this->di, Nx = this->nx;
	MC33_real Vt[12];
	MC33_real *v2 = Vt;
	this->v = Vt + 4;
	for (unsigned int z = 0; z != nz; ++z)
	{
		...
		for (unsigned int y = 0; y != ny; ++y)
		{
			...
			// TODO: there seems to be problem here
			if (signbf(v2[0])) i |= 8;
			if (signbf(v2[1])) i |= 4;
			if (signbf(v2[2])) i |= 2;
			for (unsigned int x = 0; x != Nx; ++x)
			{
				...
				if (signbf(v2[2])) i |= 2;
				if (i && i^0xFF)
				{ //i is different from 0 and 0xFF
					if (v2 < this->v)
					{
						MC33_real *t = v2;
						MC33_real *s = t + 8;
						...
						*(++s) = *(++t);
						*(++s) = *(++t);
					}// memcpy(v2 + 8, v2, 4*sizeof(MC33_real));
					find_case(x, y, z, i);
				}
			}
		}
		std::swap(this->Dx, this->Ux);
...
size_t MC33::size_of_isosurface(MC33_real iso, unsigned int &nV, unsigned int &nT) {
	...
	for (unsigned int z = 0; z != nz; ++z) {
		...
		swap(this->Dx, this->Ux);
		swap(this->Dy, this->Uy);
	}
	nV = Sf.nV;
	nT = Sf.nT;
