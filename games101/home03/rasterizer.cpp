...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos)
{
    ...
    auto v = t.toVector4();
    Vector3f normal_coor[3];
    for(int i = 0; i < v.size(); i++)
    {
        normal_coor[i] = {v[i].x(), v[i].y(), v[i].z()};
    }
    Vector3f normal_a, normal_b, normal_c;
    normal_a = normal_coor[1].cross(normal_coor[0]);
    normal_b = normal_coor[2].cross(normal_coor[1]);
    normal_c = normal_coor[0].cross(normal_coor[2]);
    ...
    for(int x = min_x_i; x < max_x_i; x++)
    {
        for(int y = min_y_i; y < max_y_i; y++)
        {
            ...
            if(insideTriangle(center_x_f, center_y_f, t))
            {
                ...
                // auto interpolated_normal = alpha * normal_a + beta * normal_b + gamma * normal_c;
                ...
                auto temp_interpolated_texcoords = alpha * view_pos[0] + beta * view_pos[1] + gamma * view_pos[2];
                // auto temp_interpolated_texcoords = alpha * t.v[0] + beta * t.v[1] + gamma * t.v[2];
                Eigen::Vector3f interpolated_shadingcoords{temp_interpolated_texcoords.x(), temp_interpolated_texcoords.y(), temp_interpolated_texcoords.z()};
                ...
                // if(this->depth_buf[(x - min_x_i) * height + (y - min_y_i)] >= zp)
                // {
                //     set_pixel(point, pixel_color);
                //     this->depth_buf[(x - min_x_i) * height + (y - min_y_i)] = zp;
                // }
            }
        }
    }
