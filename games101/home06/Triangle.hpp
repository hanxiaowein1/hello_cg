...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (v < 0 || u + v > 1)
        return inter;
    t_tmp = dotProduct(e2, qvec) * det_inv; // this is tnear
    if(t_tmp < 0.0f)
    {
        return inter;
    }
    ...
    // inter.distance = std::sqrt(
    //     std::pow(inter.coords.x - ray.origin.x, 2) +
    //     std::pow(inter.coords.y - ray.origin.y, 2) +
    //     std::pow(inter.coords.z - ray.origin.z, 2));
    inter.obj = this;
    inter.m = this->m;
    return inter;
}
